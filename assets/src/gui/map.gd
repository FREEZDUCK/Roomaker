...
func _process(delta):
	...
	icons = $rooms_icon.get_children()
	
	$rooms_icon.position = -Vector2(Info.room_in_player_pos.x / x_ratio, Info.room_in_player_pos.y / y_ratio)
...
func load_map_icon():
	...
	for room in rooms:
		...
		$rooms_icon.add_child(mini_room)
	icons = $rooms_icon.get_children()
